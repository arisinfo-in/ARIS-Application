...
export async function speechAnalysis(req: Request, res: Response) {
  ...
  try {
    ...
    if (!audioData || !question) {
      res.status(400).json({ error: 'Missing required fields: audioData and question' });
      return;
    }
...
async function performSpeechAnalysis(apiKey: string, audioData: string, question: string, difficulty: string, module: string) {
  try {
...
async function simulateSpeechToText(audioData: string, question: string): Promise<string> {
...
async function analyzeTranscription(apiKey: string, transcription: string, question: string, difficulty: string, module: string) {
  try {
...
function createAnalysisPrompt(transcription: string, question: string, difficulty: string, module: string): string {
...
function generateSuggestions(analysis: any, difficulty: string): string[] {
