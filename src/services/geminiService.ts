...
class GeminiService {
  private aiTutorApiKey: string;
  private practiceTestApiKey: string;
  private baseUrl = 'https://api.groq.com/openai/v1/chat/completions';

  constructor() {
    // AI Tutor API Key
    this.aiTutorApiKey = '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';
    // Practice Test API Key  
    this.practiceTestApiKey = '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';
    
    if (!this.aiTutorApiKey && !this.practiceTestApiKey) {
      throw new Error('No Groq API keys found');
    }
  }

  private async makeApiCall(prompt: string, isDynamicTest: boolean = false): Promise<string> {
    // Use appropriate API key based on functionality
    const apiKey = isDynamicTest ? this.practiceTestApiKey : this.aiTutorApiKey;
    const keyType = isDynamicTest ? 'Practice Test' : 'AI Tutor';
    
    console.log(`Using ${keyType} API key: ${apiKey.substring(0, 10)}...`);
    ...
    try {
      const response = await fetch(this.baseUrl, {
        ...
        body: JSON.stringify({
          ...
          temperature: 1,
          max_completion_tokens: isDynamicTest ? 8192 : 4096,
          top_p: 1,
      ...
      if (!response.ok) {
        ...
        if (errorData.error?.code === 429) {
          ...
          throw new Error(`${keyType} API quota exceeded. Please try again in ${retryAfter} or upgrade your API plan.`);
        }
        
        console.error(`${keyType} API Error:`, errorText);
      ...
      console.error(`Error with ${keyType} API:`, error);
      throw error;
    }
  }
  ...
  async generateResponse(prompt: string, module: string, conversationHistory: Array<{ content: string; isUser: boolean }> = []): Promise<string> {
    try {
      ...
      return await this.makeApiCall(fullPrompt, false);
    } catch (error) {
  ...
  async generateDynamicTest(systemPrompt: string): Promise<string> {
    try {
      console.log('Generating dynamic test with fallback API keys...');
      return await this.makeApiCall(systemPrompt, true);
    } catch (error) {
      console.error('Error calling Groq API for dynamic test:', error);
